...
const HeatMapOverLayer = L.Layer.extend({
  ...
  _reDraw: function () {
    ...
    // 获取半径
    let radius = this._options.radius * scale
    ...
    let renderData = filterData.map((item) => {
      let point = this._map.latLngToContainerPoint([item[1], item[0]])

      let arr = [Math.floor(point.x), Math.floor(point.y), item[2]]
      return arr
    })

    // 插值
    console.time('插值')
    let interpolate = bilinearInterpolation(this._width, this._height, renderData)
    console.timeEnd('插值')
    // 遍历数据画出阴影圆形
    ...
    for (let i = 0; i < this._height; i++) {
      for (let j = 0; j < this._width; j++) {
        let alpha = interpolate[i][j]
        let color = gradient.colorPicker(alpha)
        colorData[(i + j) * 4] = color.r
        colorData[(i * j) * 4 + 1] = color.g
        colorData[(i * j) * 4 + 2] = color.b
        colorData[(i * j) * 4 + 3] = color.a
      }
    }
    // // 遍历数据
...
function bilinearInterpolation(width, height, knownPoints) {
  ...
  function findNeighbors(x, y) {
    ...
    for (let point of knownPoints) {
      ...
      if (px <= x && px + 1 >= x && py <= y && py + 1 >= y) {
        neighbors.push({ x: px, y: py, value });
      }
    }
    // 根据x, y排序确保左上角、右上角、左下角、右下角  
    neighbors.sort((a, b) => (a.x - b.x) || (a.y - b.y));
    return neighbors;
  }
